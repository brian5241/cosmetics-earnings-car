...
```mermaid
flowchart TD
    DART[("DART OpenAPI<br/>공시 · 재무제표")]
    KRX[("FinanceDataReader / pykrx<br/>주가 · 지수")]

    subgraph collect ["수집"]
        S01["01 · 유니버스 검증<br/>11종목 + v2 후보"]
        S02["02 · 고유번호 매핑"]
        S03["03 · 공시 목록<br/>1,453건"]
        S05["05 · 재무제표<br/>7,035행"]
        S07["07 · 주가 · 지수<br/>13,527행"]
    end

    subgraph clean ["정제 · 검증"]
        S04["04 · 이벤트 테이블<br/>160건"]
        S06["06 · 분기 패널<br/>246행 · 결측 0"]
        V06{{"검산 · 54건 전부 일치<br/>Q1+Q2+Q3+Q4 = 연간"}}
    end

    subgraph analyze ["분석"]
        S08["08 · CAR 계산<br/>2팩터 시장모델"]
        V09{{"검증 · 원본 종가 대조<br/>극단값 전부 실제 거래"}}
        S10["10 · 특성 생성<br/>최종 표본 155건"]
        S11["11 · OLS + Lasso"]
    end
    ...
    subgraph output ["산출"]
        ...
        S13["13 · 보고서"]
        S14["14 · SQLite 적재"]
        V14{{"SQL 교차 검증<br/>pandas 결과와 일치"}}
        NB["EDA 노트북"]
    end
    ...
    classDef check stroke-dasharray: 4 3,stroke-width:1.5px
    class V06,V09,V14 check
```

점선 육각형은 **검증 단계**다. 계산한 값을 그대로 넘기지 않고
원자료 또는 다른 계산 경로와 대조한 지점이다.

두 데이터 소스가 `08 · CAR 계산`에서 처음 만난다.
여기서 발표 시점을 거래일에 정확히 맞추는 것이 이 파이프라인의 핵심이다.
